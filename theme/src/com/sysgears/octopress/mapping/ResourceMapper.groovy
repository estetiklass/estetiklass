        ...
        resources.inject([]) { List updatedResources, Map page ->
            ...
            switch (page.url) {
                ...
                case ~/\/blog\/.*/:
                    ...
                    break
                case '/contacts/masters/':
                    def masters = resources.findAll { ('master' == it.layout) }
                    updatedResources << (page + ['masters' : masters])
                    break
                case '/learning/classes/':
                    def classes = resources.findAll { ('class' == it.layout) }
                    updatedResources << (page + ['classes' : classes])
                    break
                case ~/\/products\/.*/:
//                case '/products/':
                    def classes = resources.findAll { ('product' == it.layout) }.groupBy { it.line }
                    updatedResources << (page + ['products' : classes])
                    break
                default:
