...
class OctopressTagLib {
    ...
    def parseDate = { Map attrs ->
        if (!attrs.date) throw new IllegalArgumentException('Tag [xmlDateTime] is missing required attribute [date]')

        Date.parse("yyyy-MM-dd hh:mm", attrs.date)
    }
    ...
    private def getContent(String fileName) {
        def yml = new File(taglib.site.content_dir as String, "_site/${fileName}")
        return taglib.site.headerParser.parse(yml, yml.text)
    }
    def dashboardTiles = {
        getContent('dashboard.yml')
    }

    def contacts = {
        getContent('contacts.yml')
    }

    def publications = {
        getContent('publications.yml')
    }

    def aboutus = {
        getContent('aboutus.yml')
    }
}
